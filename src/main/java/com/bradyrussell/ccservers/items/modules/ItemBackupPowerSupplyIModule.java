...
public class ItemBackupPowerSupplyIModule extends ItemServerModuleBase {
    ...
    public ItemBackupPowerSupplyIModule(EServerModuleType type) {
        super(type);
    ...
    public void onTick(ItemStack moduleItem, TileEntityServerChassis serverChassis) {
        ...
        if(serverChassis.getEnergyStorage().getEnergyStored() < serverChassis.getEnergyStorage().getMaxEnergyStored()*BACKUP_PSU_ACTIVATION_THRESHOLD){
            ...
            if(energy > 0) setStoredEnergy(moduleItem, energy - serverChassis.getEnergyStorage().receiveEnergy(Math.min(maxCurrentDraw*5, energy), false));
    ...
    public static void setStoredEnergy(ItemStack itemStack, int energy){
        itemStack.getItem().setDamage(itemStack, (int) (energyDurabilityBarFactor-((double)energy/((ItemBackupPowerSupplyIModule)itemStack.getItem()).getEnergyCapacity()*energyDurabilityBarFactor)));
        itemStack.setStackDisplayName("Backup PSU: "+energy+" / "+((ItemBackupPowerSupplyIModule)itemStack.getItem()).getEnergyCapacity()+" RF");
