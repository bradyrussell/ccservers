...
public class ItemBackupPowerSupplyModule extends ItemServerPoweredLuaModuleBase {
    ...
    private TileEntityServerChassis serverChassis;
    ...
    public void onTick(ItemStack moduleItem, TileEntityServerChassis serverChassis) {  // this module has custom behavior because it doesnt just charge, it also releases
        ...
        this.serverChassis = serverChassis;
    }
    ...
    public String[] getMethodNames() {
        return new String[]{"getStoredEnergy","releaseStoredEnergy"};
    }
    ...
    public Object[] callMethod(ItemStack moduleStack, IComputerAccess computer, @Nonnull ILuaContext luaContext, int method, @Nonnull Object[] arguments) throws LuaException, InterruptedException {
        switch (method) {
            ...
            case 1:{
                if (arguments.length != 1) throw new LuaException("Expected 1 parameter: releaseStoredEnergy(Number amount)");
                if (!(arguments[0] instanceof Double)) throw new LuaException("Expected number for parameter 1: releaseStoredEnergy(Number amount)");
                int extract = (int) Math.floor((Double)arguments[0]);
                int actual=0;
                if (ModuleEnergy.getEnergy(moduleStack) >= extract && serverChassis != null)
                    actual = serverChassis.getEnergyStorage().receiveEnergy(extractEnergy(moduleStack, extract),false);
